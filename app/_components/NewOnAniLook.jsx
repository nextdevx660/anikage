...
export default function LatestEpisodes() {
  ...
  const getLatestEp = async () => {
    const res = await axios.get('https://anime-api-zeta-hazel.vercel.app/api/recently-added')
    setLatestEps(res?.data?.results?.data)
  }

  return (
    <div className='px-3 mt-5'>
      <h2 className='text-2xl text-rose-500 font-bold py-3 flex items-center justify-between'>New on AniKage
        <span>
          <Link href={'/recently-added'} className='flex items-center gap-2 text-gray-500 text-[15px]'>
            View More
